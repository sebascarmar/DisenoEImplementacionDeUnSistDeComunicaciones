...
SNR_db   = 7
f_offset = 00e3 # Hz
NSYMB_CONVERGENCE = 500000   # FSE and FCR convergence (a half for each)
...
del tx_bitI_prbs  # Elimina el array  
del tx_bitQ_prbs  # Elimina el array  
gc.collect()  # Fuerza la liberación de memoria  
...
del tx_symI_up  # Elimina el array  
del tx_symQ_up  # Elimina el array  
gc.collect()  # Fuerza la liberación de memoria  
...
del tx_symI_rrc  # Elimina el array  
del tx_symQ_rrc  # Elimina el array  
gc.collect()  
...
del ch_symI_noisy  # Elimina el array  
del ch_symQ_noisy  # Elimina el array 
gc.collect()  
...
del ch_symI_rot  # Elimina el array  
del ch_symQ_rot  # Elimina el array
gc.collect() 
...
del ch_symI_ch_filt  # Elimina el array  
del ch_symQ_ch_filt  # Elimina el array 
gc.collect()

#### Downsampler (rate 2)
rx_symI_dw_r2 = rx_symI_aaf[0:len(rx_symI_aaf):int(OS_DSP)]
rx_symQ_dw_r2 = rx_symQ_aaf[0:len(rx_symQ_aaf):int(OS_DSP)]
del rx_symI_aaf  # Elimina el array  
del rx_symQ_aaf  # Elimina el array 
gc.collect()  
...
del rx_symI_dw_r2  # Elimina el array  
del rx_symQ_dw_r2  # Elimina el array 
gc.collect()  

#### DSP
# FSE variables
fseI_buffer = np.loadtxt('cont_fseI_buffer.txt'  , delimiter=',')
fseQ_buffer = np.loadtxt('cont_fseQ_buffer.txt'  , delimiter=',')
fseI_coeff  = np.loadtxt('cont_fseI_coeff.txt'  , delimiter=',')
fseQ_coeff  = np.loadtxt('cont_fseQ_coeff.txt'  , delimiter=',')
...
# FCR variables
FCR_DATA = np.loadtxt('cont_fcr_data.txt'  , delimiter=',')
prop_err    = FCR_DATA[0]
int_err     = FCR_DATA[1]
nco_out     = FCR_DATA[2]
...
np.savetxt('fse_coeff_150k.txt' , fse_coeff[:,50000-1]      , delimiter=',')
np.savetxt('fse_coeff_200k.txt' , fse_coeff[:,100000-1]      , delimiter=',')
# np.savetxt('fse_coeff_120k.txt' , fse_coeff[:,120000-1]      , delimiter=',')
# np.savetxt('rx_symI_dw_r2.txt' , rx_symI_dw_r2      , delimiter=',')
# np.savetxt('rx_symI_agc.txt' , rx_symI_agc      , delimiter=',')
# np.savetxt('rx_symI_fse.txt' , rx_symI_fse      , delimiter=',')
# np.savetxt('rx_symI_dw_r1.txt' , rx_symI_dw_r1      , delimiter=',')
...
plt.ylim(-0.3,3)
plt.grid(True)
plt.savefig("fse_coeff_b.png", dpi=300, format='png')
plt.close()
#plt.show()
