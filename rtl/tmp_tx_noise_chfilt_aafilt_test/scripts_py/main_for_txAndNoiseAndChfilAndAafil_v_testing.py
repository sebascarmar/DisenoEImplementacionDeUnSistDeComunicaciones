...
rx_symI_aa_filt_from_ver = np.loadtxt("file_rx_symI_aafilt.txt", dtype=np.intp)
rx_symQ_aa_filt_from_ver = np.loadtxt("file_rx_symQ_aafilt.txt", dtype=np.intp)
#convierte al valor correspondiente al formato de punto fijo S(8.7)
frac_bits = 7
rx_symI_aaf = rx_symI_aa_filt_from_ver / (2**frac_bits)
rx_symQ_aaf = rx_symQ_aa_filt_from_ver / (2**frac_bits)
print("Fragmento de salidas del filtro antialias I y Q")
print(rx_symI_aaf[19:29])
print(rx_symQ_aaf[19:29])
...
print("\n Datos pre y post downsampler")
print(rx_symI_aaf[0:20])
print(rx_symI_dw_r2[0:10], "\n")
...
for j in range(NSYMB*OS_DSP):
    ...
    if((j+1)%OS_DSP)==0: # Downsampling to BR rate (os=1)
        ...
        # PI loop filter
        Kp2 = Kp if(k>(NSYMB_CONVERGENCE/2)) else 0
        Ki2 = Ki if(k>(NSYMB_CONVERGENCE/2)) else 0
