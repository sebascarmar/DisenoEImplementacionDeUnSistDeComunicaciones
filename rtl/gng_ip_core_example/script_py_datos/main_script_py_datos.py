...
#output_folder = "/home/danielito/Escritorio/Plots_constelation_float"
path_logs = "/home/danielito/Escritorio/Logueos/"

#Crear la carpeta si no existe (opción 1: usando subprocess)
#subprocess.run(["mkdir", "-p", output_folder])
# Opción alternativa sin subprocess (más pythonico)
...
### LOGUEO DE DATOS DESDE VERILOG 
path_noise_I = f"/home/danielito/Escritorio/run/GNG_Prototype/Modules/logs/noise_I_SNR_{snr}.txt"
path_noise_Q = f"/home/danielito/Escritorio/run/GNG_Prototype/Modules/logs/noise_Q_SNR_{snr}.txt"

path_noise_fres_I = f"/home/danielito/Escritorio/run/GNG_Prototype/Modules/logs/noise_fres_I_SNR_7.txt"
path_noise_fres_Q = f"/home/danielito/Escritorio/run/GNG_Prototype/Modules/logs/noise_fres_Q_SNR_7.txt"

### Load the data
try:
    noise_I_from_gng_v = np.loadtxt(path_noise_I, dtype=np.int0)
    noise_Q_from_gng_v = np.loadtxt(path_noise_Q, dtype=np.int0)
    # noise_I_from_gng_v = np.loadtxt(path_noise_fres_I, dtype=np.int0)
    # noise_Q_from_gng_v = np.loadtxt(path_noise_fres_Q, dtype=np.int0)    
except Exception as e:
    print(f"Error loading files: {e}")
    exit()

### Convert from S(8,7) or S(26,17)
frac_bits = 7
# frac_bits = 17
noise_I_v = noise_I_from_gng_v / (2 ** frac_bits)
noise_Q_v = noise_Q_from_gng_v / (2 ** frac_bits)



for SNR_db in range(snr, (snr+1)):
    ############################## TRANSMITTER  #############################
    ...
    # print(noise_I_v[100000:100050])
    # print(max(noise_I_v))
    # print(min(noise_I_v))
    # print()
    # ch_symI_noisy = tx_symI_txf + noise_I_v
    # ch_symQ_noisy = tx_symQ_txf + noise_Q_v
...
plt.title('FCR output constellation')
plt.plot(rx_symI_fcr[len(rx_symI_fcr)-40000:],
        rx_symQ_fcr[len(rx_symQ_fcr) -40000:],
...
plt.title('FSE coefficients (I lane) - decimated - With verilog data')
plt.grid(True)
#plt.show()
